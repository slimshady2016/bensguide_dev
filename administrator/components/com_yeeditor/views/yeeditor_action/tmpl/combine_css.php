...
if($_POST['action']=="combine_css"){
	...
	foreach($folders as $folder){
		//
		$css_path = $widget_ex_fpath.$folder."/frontend/assets/css/".$folder.".css";
		if(file_exists($css_path)){
			...
			$temp_content = str_replace("../images/", "../../../../../components/com_yeeditor/widgets_ex/".$folder."/frontend/assets/images/", $temp_content);
	...
	$replace_str='/* widget margin bottom */'."\n".'div.yeeditor .yee-widget{margin-bottom:'.$margin_bottom.';}';
	$css_content=file_get_contents($docs_css_path);
	$css_new_content=preg_replace('/\/\* widget margin bottom \*\/[\s\S]*?div.yeeditor .yee-widget{margin-bottom:.*?;}/',$replace_str,$css_content);
