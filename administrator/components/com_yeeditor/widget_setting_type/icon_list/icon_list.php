...
(function($){
	$(function(){
			...
			function add_item(element,name,number){
				...
				var num=$('div[data-name="'+name+'"] ul li:not(.add_new_item)').length+1;
					
				var html_str='<li class="single-item"><i class="icon-resize-vertical"></i>';
				
				for(var i = 0; i < number; i ++){
					if(i==0){
						html_str = html_str + '<input type="text" name="'+name+'_'+num+'" onclick="select_image(\''+name+'_'+num+'\')"  style="width:'+(1/number*75)+'%" class="item_data" value="" onchange="get_data(\''+name+'\')"/>';
					}
					else{
				...
				if(data_arr){
					...
					for(var i = 0; i < data_arr.length; i ++){
						...
						for(var k = 0; k < data_arr[i].length; k ++){
							if(k==0){
								html_str = html_str + '<input type="text" name="'+name+'_'+i+'" onclick="select_image(\''+name+'_'+i+'\')" style="width:'+(1/number*75)+'%" class="item_data" value="'+data_arr[i][k]+'" onchange="get_data(\''+name+'\')"/>';
							}
							else{
			...
			window.select_image = function(item_name){
					window.open ("../plugins/editors/yeeditor/assets/js/ckeditor/elfinder/elfinder_select_image.html?image="+item_name,"newwindow","height=500,width=1100,top=" + (window.screen.availHeight-30-500)/2 +",left=" + (window.screen.availWidth-10-1100)/2 +",toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no") ;
			}
