...
class YeeditorControllerTemplates extends JControllerAdmin
{
	protected $text_prefix = 'COM_YEEDITOR_FIELD_EXTENSIONS';

	public function getModel($name = 'Template', $prefix = 'YeeditorModel', $config = array('ignore_request' => true))
	{
	...
	public function remove()
	{
	   ...
	   foreach($post['cid'] as $id){
			...
			$sqlquery = "select widget_name,setting_type from #__yeeditor where id=".$id;
			...
			$sqlquery="";
			$sqlquery = "delete from #__yeeditor where id=".$id;
			$database->setQuery( $sqlquery );
			if (!$result = $database->query()) {
				echo $database->stderr();
				return false;
	   ...
	   $this->setRedirect('index.php?option=com_yeeditor&view=templates&layout=default&task=remove');
	}
}