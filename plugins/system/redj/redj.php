  ...
  private static function manageMacroParams($macro)
  {
    ...
    switch ($macro[1])
    {
      ...
      case 'requestvar':
        ...
        if (isset($macro[3]))
        {
          $value = JFactory::getApplication()->input->get($macro[2], $macro[3]);
        } else {
          $value = JFactory::getApplication()->input->get($macro[2]);
        }
        break;
      case 'pathltrim':
        $value = $urlpaths['path'];
  ...
  private static function replaceMacros($content)
  {
    ...
    */
    ...
    if (!isset($patterns_callback))
    {
      ...
      $patterns_callback[31] = "/\{(preg_subject) ([^\}]+)\}(.*)\{\/preg_subject\}/suU";
      $patterns_callback[32] = "/\{(preg_placeholder) ([^\}]+)\}(.*)\{\/preg_placeholder\}/suU";
      $patterns_callback[33] = "/\{(lowercase)\}(.*)\{\/lowercase\}/suU";
      $patterns_callback[34] = "/\{(uppercase)\}(.*)\{\/uppercase\}/suU";
      $patterns_callback[35] = "/\{(urldecode)\}(.*)\{\/urldecode\}/suU";
      $patterns_callback[36] = "/\{(urlencode)\}(.*)\{\/urlencode\}/suU";
      $patterns_callback[37] = "/\{(rawurldecode)\}(.*)\{\/rawurldecode\}/suU";
      $patterns_callback[38] = "/\{(rawurlencode)\}(.*)\{\/rawurlencode\}/suU";
    ...
    if ( !empty($this->_tourl) )
    {
      ...
      switch ($this->_redirect) {
        ...
        case 200:
          ...
          self::doInternalRedirect($this->_baseonly, $this->_visited_full_url, $this->_tourl);
          break;
        default:
          header("HTTP/1.1 301 Moved Permanently");
          header('Location: ' . $this->_tourl);
