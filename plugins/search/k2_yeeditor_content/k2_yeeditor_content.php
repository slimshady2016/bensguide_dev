...
class plgSearchK2_yeeditor_content extends JPlugin
{

	public function __construct($subject, $config)
	{
		$db = JFactory::getDbo();
		$db->setQuery("SELECT extension_id FROM #__extensions WHERE name = 'Search - K2'");
		$id = $db->loadResult();
		if($id){
    ...
    function onSearch($text, $phrase = '', $ordering = '', $areas = null)
    {
        ...
        $plugin = JPluginHelper::getPlugin('search', 'k2_yeeditor_content');
        ...
        //translate article yeeditor content
		if(file_exists(JPATH_PLUGINS."/editors/yeeditor/include/functions_frontend.php")){
			require_once JPATH_PLUGINS."/editors/yeeditor/include/functions_frontend.php";
			
			$yee_results = $results;
			foreach($results as $key=>$value){
				$yee_article_text = $value->text;
				
				$yee_article_text = get_yeeditor_translated_html($yee_article_text);
				
				$yee_results[$key]->text = 	$yee_article_text;
			}
